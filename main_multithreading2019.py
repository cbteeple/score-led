...
#Set up the colors for twitter sentiment
#thresh_pos = 95
#thresh_neg =70
...
if __name__ == '__main__':
	...
	try:
		...
		while 1:	
			...
			thresh_neg = thresholds['thresh_neg']
			thresh_pos = thresholds['thresh_pos']

			if positivity[team1] <= thresh_neg:
				R=255
				G=0
				B=0
			elif positivity[team1] >= thresh_pos:
				R=0
				G=255
				B=0
			else:
				R=255-(positivity[team1]-thresh_neg)*255/(thresh_pos-thresh_neg)
				G=(positivity[team1]-thresh_neg)*255/(thresh_pos-thresh_neg)
				B=0

			color_vec[1]= [math.floor(R),math.floor(G),B]
			
			#R=25.5*math.sqrt(-positivity[team2]+100)
			#G=255./10000*(positivity[team2]**2)
			#B=0

			if positivity[team2] <= thresh_neg:
				R=255
				G=0
				B=0
			elif positivity[team2] >= thresh_pos:
				R=0
				G=255
				B=0
			else:
				R=255-(positivity[team2]-thresh_neg)*255/(thresh_pos-thresh_neg)
				G=(positivity[team2]-thresh_neg)*255/(thresh_pos-thresh_neg)
				B=0


			color_vec[3]= [math.floor(R),math.floor(G),B]
